...
As of version 0.1, in April 2020, lfs_py comes configured for 3 versions of
the LFS book:
...
Using lfs_py to build your LFS system is accomplished in just a few steps:
...
https://github.com/joaomdsc/pkg. Cd to lfs inside the directory where you
cloned.

lfs_py uses the 'bc' command for SBU calculations in the shell scripts, you
...
Set the variable values in file lfs/config.py. The following variables are
...
lfs_py needs some place on your computer to keep files around in between
...
By default, lfs_py will use **$HOME/lfs** for LFS_DATA. If you want it to use
...
lfs_py will also use **/var/tmp/lfs/{version}** as a place to persist files that
...
You are now ready to generate your python scripts. Cd to lfs in the directory
where you cloned the repository. Run the following command as a non-privileged
user: ::
...
the lfs_py scripts. Select it, and press enter to boot:
...
The scripts generated by lfs_py will also save the kernel's .config file to
...
lfs_py parses the HTML file of the LFS book, extracts all the scripts, and
...
case, the lfs_py python code will generate shell script code that tests
